...
-- VARCHAR타입의 숫자값이 NUMBER 타입으로 형변환이 일어나면서 저장된다.
...
-- DNO이 30인 사원목록을 EMP 테이블에서 조회해서 EMP_DNO30에 저장
...
-- DNO이 10인 사원의 사원번호, 사원이름만 EMP 테이블에서 조회해서 EMP_DNO30에 저장
...
-- 모든 과목의 과목번호, 과목이름, 학점, 담당하는 교수의 교수번호, 교수이름을 COURSE_PRFESSOR에 저장
-- 담당교수가 없는 과목은 담당교수 번호와 이름을 NULL로 저장
...
-- EMP_DNO30 테이블에서 ENO이 3004인 사원의 업무를 경영으로 MGR은 2003으로 HDATE는 오늘 날짜로 SAL은 5000으로
-- COMM는 500으로 수정
UPDATE EMP_DNO30 
	SET 
		JOB = '경영',
...
-- EMP 테이블에서 최고 급여를 받는 사원으로 수정 => 아이유 7200 1002
UPDATE DEPT_COPY1 
	SET 
		DIRECTOR = (
					SELECT ENO
...
-- WHERE절에 서브쿼리로 특정 데이터를 지정할 수도 있다.
DELETE FROM EMP_DNO30
	WHERE ENO IN (
		SELECT ENO
			FROM EMP_DNO30

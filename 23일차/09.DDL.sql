...
-- CREATE 객체명(TABLE, USER, INDE, SEQUENCE...) 객체의 별칭 (테이불명, 유저명, 인덱스명, 시퀸스명...)
-- 다른 테이블을 참조하여 테이블 생성
...
SELECT *
	FROM COURSE_PRFESSOR_AVGRES;

-- 학생번호, 학생이름, 학생별 기말고사 성적의 평균점수(소수점 둘째자리), 학생별 기말고사의 평균 점수의 등급을 가지는
...
SELECT ST.SNO
		...
		GROUP BY ST.SNO, ST.SNAME, SG.GRADE;

CREATE TABLE ST_AVGRES_GRADE(STU_NO, STU_NAME, STU_AVG, STU_GRADE)
	AS SELECT A.SNO
			, A.SNAME
			, A.AVG_RES
			, SCG.GRADE
...
-- DROP 객체명(TABLE, INDEX, SEQENCE, USER, ...) 객체의 별칭(테이블명, 유저명, 인덱스명, 시퀸스명)
...
-- TRUNCATE는 DDL이기 때문에 트랜잭션이 바로 종료되어 ROLLBACK으로 작업을 취소할 수 없다.

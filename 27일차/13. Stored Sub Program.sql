...
-- 기본값이 있는 파라미터는 생략 가능
-- DBeaver에서는 EXEC 를 사용하지 않고 CALL, BEGIN~END를 사용한다.
CALL P_NEW_DEPT('80', '테스트');

SELECT * FROM DEPT;

-- 1-3. Stored Function : 오라클 사용자 정의 함수, 프로시저와 동일한 역할을 하지만 
-- 						  프로시저와 다르게 리턴값이 존재해서 EXEC 명령어로 단독 실행도 되고
-- 						  일반 쿼리문에서 호출도 가능하다.
-- 급여별로 세금을 셰산해주는 함수 F_GET_TAX
...
SELECT E.ENO
	, E.ENAME 
	, E.SAL 
	, F_GET_TAX(E.SAL) AS TEX
	FROM EMP E;
	
-- 평점을 파라미터로 받아서 4.5만점인 평점으로 환산하는 F_CONVERT_AVR을 만들어보세요.
-- 세미콜론, ROUND에 2자리수까지 출력하는 걸 넣지 않아서 실행이 안됐음
...
IS 
	CONVERT_AVR NUMBER;
BEGIN 
	CONVERT_AVR := ROUND(((AVR / 4.0) * 4.5), 2);
	RETURN CONVERT_AVR;
END;

SELECT ST.SNO
	, ST.SNAME
	, F_CONVERT_AVR(ST.AVR) AS AVR
	FROM STUDENT ST;
...
SELECT ST.SNO
	...
	, F_CONVERT_AVR(ST.AVR) AS AVR
	FROM STUDENT ST
	WHERE F_CONVERT_AVR(ST.AVR) >= 4.2;
...
IS
	F_PLUS_10YEARS DATE
BEGIN 
	PARAM_DATE := ADD_MONTHS(PARAM_DATE, 120); 
	RETURN PARAM_DATE;
END;
...
SELECT PNO
	, PNAME
	, HIREDATE
	, F_PLUS_10YEARS AS "부임일 10년뒤"
	FROM PROFESSOR;


-- 두 날짜를 파라미터로 받아서 두 날짜의 차이를 일자로 리턴하는 함수 F_DIFF_DATES를 만드세요.
CREATE OR REPLACE FUNCTION F_DIFF_DATESS(
	DATE1 DATE,
	DATE2 DATE
)
RETURN NUMBER 
...
SELECT ENO
	, ENAME
	, HDATE
	, F_DIFF_DATESS(SYSDATE, HDATE) "입사후 지난일자"
	FROM EMP;

-- 1-4. TRIGGER : 특정 테이블의 데이터 변경이 있을 때 지정한 PL/SQL 구문을 실행하는 SUB PROGRAM
-- BEFORE TRIGGER
...
-- 에러코드 발생, 20001-최저 시급보다 낮은 급여는 추가할 수 없습니다. 
INSERT INTO EMP 
...
-- 에러코드 발생, 20002-최저 시급보다 낮은 급여로 데이터를 수정할 수 없습니다.
UPDATE EMP 
	SET 
		SAL = 2900
	WHERE ENO = '9999';

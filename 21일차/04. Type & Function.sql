...
-- CONCAT 함수는 매개변수를 두 개만 받을 수 있어 여러개의 데이터나 다른 표현을 넣고 싶을 때는 ||와 조합하여 사용한다.
SELECT CONCAT(SNAME, ': ' || AVR) 
	FROM STUDENT;
...
-- SUBSTR(데이터, 시작인덱스, 개수) : 받아온 데이터에서 시작 인덱스에서 개수만큼 데이터를 잘라 리턴
...
SELECT ENAME 
	...
	, SUBSTR(ENAME, -2, 2) -- 뒤에서 두 번째 글자부터 두글자  
	FROM EMP;
	
-- LENTH : 데이터의 길이를 리턴
-- LENTHB :  데이터의 길이를 BYTE 단위로 리턴
-- 오라클의 기본 문자셋은 AL32UTF8 => 영어(1BYTE)를 제외한 다른 문자는 3BYTE 씩 계산
SELECT DNAME
	, LENGTH (DNAME)
	, LENGTHB(DNAME) 
	FROM DEPT;
...
-- TRIM(데이터) : 데이터에서 앞, 뒤의 공백을 제거
SELECT TRIM(LEADING '0' FROM '00012300') -- 앞에 있는 0 제거
	 , TRIM(TRAILING '0' FROM '00012300') -- 뒤에 있는 0 제거
	 , TRIM(BOTH '0' FROM '00012300') -- BOTH는 생략 가능, 앞 뒤 0 제거
	 , TRIM('    0 0 0 1 2 3 0 0 0    ') -- 앞 뒤 공백 제거
	 FROM DUAL;
...
-- 사원의 사원번호, 사원이름을 조회하는 데 사원이름에서 마지막 한 글자만 빼고 조회
SELECT ENO  
	, ENAME
...
-- POWER(데이터1, 데이터 2) : 데이터1의 데이터2 제곱한 값을 리턴 (데이터1의 데이터2승)
SELECT POWER(3, 3)
	FROM DUAL;
	
-- CEIL(데이터) : 데이터보다 큰 정수 중 가장 작은 정수 리턴
-- FLOOR(데이터) : 데이터보다 작은 정수 중 가장 큰 정수
SELECT CEIL(2.59)
	, FLOOR(2.59)
	FROM DUAL;
...
-- 1-5. 날짜 연산
SELECT SYSDATE 
	 , SYSDATE + 100 -- 오늘 날짜부터 100일뒤
	 , SYSDATE - 100 -- 오늘 날짜부터 100일 전
	 , SYSDATE + 3/24 -- 현재 시간으로부터 3시간 뒤
	 , SYSDATE - 5/24 -- 현재 시간으로부터 5시간 전
	 , SYSDATE - TO_DATE('20210523', 'YYYYMMDD') -- 두 날짜간의 차이
	 -- 날짜에 TRUNC 함수를 사용하면 시간 분 초를 00:00:00으로 초기화한다.
	 , TRUNC(SYSDATE) - TO_DATE('20210523', 'YYYYMMDD')
	 FROM DUAL;

-- 1999년 1월 1일부터 100일뒤까지 부임한 교수의 교수번호, 교수이름, 부임일자 조회
SELECT PNO 
	, PNAME 
	, HIREDATE 
	FROM PROFESSOR 
...
-- 1-6. 날짜 함수
-- ROUND(날짜, 날짜 단위) : 지정한 날짜 단위까지의 날짜는 빼오고 나머지 날짜는 초기화. 날짜를 빼올 때 절반 이상 지났으면 반올림
...
-- 1-7. 변환함수
-- TO_CHAR (날짜나 숫자, 변환될 문자열의 형식 지정자) : 매개변수로 받은 날짜나 숫자 데이터를 지정된 형식으로 변환된 문자열을 리턴
-- 숫자를 문자열로 변환
SELECT TO_CHAR(10000000, '999,999,999') -- 9자리까지 숫자를 표기, 3자리마다 쉼표 표시
...
SELECT CONCAT(PNAME, '교수의 부임일은 ' || TO_CHAR(HIREDATE, 'YYYY"년 " MM"월 " DD"일입니다."'))  
	FROM PROFESSOR;
...
-- TO_DATE(문자열 데이터, 문자열 데이터의 날짜형식) :매개변수로 지정된 날짜형식을 되어 있는 문자열을 DATE 타입으로 변환 후 리턴
SELECT TO_DATE('20240524154900', 'YYYY/MM/DDHH24MISS')
...
-- TO_NUMBER(문자열 데이터, 문자열 데이터의 숫자 형식) : 매개변수로 지정된 숫자형식으로 된 문자열을 NUMBER 타입으로 변환 후 리턴
SELECT TO_NUMBER('-123.456', '999.999')
	 , TO_NUMBER('123', '999.999')
	 , TO_NUMBER('1234')
	FROM DUAL;

-- ROUND(실수, 소수점), TRUNC(실수, 소수점)
-- ROUND(실수), TRUNC(실수) : 정수까지 반올림, 정수까지 버림
SELECT ROUND(1.45, 0)
	 , ROUND(1.45)
	 FROM DUAL;
	
-- 1-8. NVL : NULL 처리 함수
-- 과목의 과목번호, 과목이름, 교수번호, 교수이름을 조회하는 데 담당교수가 배정되지 않은 과목과 과목을 배정받지 못한 교수의 정보도 함께 조회
-- 담당교수가 NULL인 데이터는 교수가 배정되지 않음이라고 조회하고
-- 과목이 없는 교수의 과목정보는 과목 배정받지 못함이라고 조회
SELECT NVL(C.CNO, '과목 배정받지 못함') AS 과목번호
	, NVL(C.CNAME, '과목 배정받지 못함') AS 과목이름
	, NVL(P.PNO, '교수가 배정되지 않음') AS 교수번호
	, NVL(P.PNAME, '교수가 배정되지 않음') AS 교수이름
	FROM COURSE C
	FULL OUTER JOIN PROFESSOR P
	  ON C.PNO = P.PNO ;
-- 1-9. DECODE(컬럼명이나 조회해온 데이터, 조건값1,
...
-- 		  컬럼의 데이터나 조회해온 데이터가 조건 값N과 일치하지 않을 때 실행될 내용
SELECT ENO
	, ENAME
	, DNO
	, DECODE(DNO, 
...
-- 업무가 개발이면 50%인상, 업무가 경영이면 30% 인상, 지원이면 20% 인상, 그 외 업무면 10% 인상
SELECT ENO 
	, ENAME 
	, JOB 
	, SAL 
...
-- 사원의 사원번호, 사원이름, 업무, 급여, 인상급여 조회
-- 업무가 개발이면 50%인상, 업무가 경영이면 30% 인상, 지원이면 20% 인상, 그 외 업무면 10% 인상 (CASE ~ WHEN)
SELECT ENO 
	, ENAME 
	, JOB
	, SAL

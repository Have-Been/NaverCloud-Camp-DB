...
-- 						   제약조건에는 PK, FK, UK, NO NULL, DEFAULT, CHECK 등이 존재한다. 
-- 1-1. PK(PRIMARY KEY) : 테이블에 식별자 역할을 하는 컬럼을 지정
--  					  PK로 지정된 컬럼의 데이터는 유일해야 하며(UK), 색인 역할도 하며 (INDEX)
-- 						  NULL 값이 저장될 수 없다(NOT NULL).
-- 단일 컬럼 PK인 테이블 생성
CREATE TABLE EMP_PK1(
	-- 제약조건명을 명시하지 않을 시 시스템에서 제약조건명을 자동으로 생성한다
...
-- PK로 설정된 데이터는 NULL값을 가질 수 없다.
INSERT INTO EMP_PK1
VALUES(NULL, '임꺽정','분석', SYSDATE, 2);

INSERT INTO EMP
VALUES(NULL, '임꺽정','분석', NULL, SYSDATE, 5000, 600, NULL);
...
-- 다중 컬럼으로 PK를 지정하게 되면 PK로 지정한 컬럼들을 데이터 셋으로 묶어서 PK로 인식한다.
...
-- PK로 지정된 모든 컬럼에 NULL값은 허용되지 않는다
INSERT INTO PROFESSOR_PK1
VALUES (1, '고기천', '물리', '부교수', SYSDATE);
...
-- 기존 테이블에 다중컬럼 PK 추가
ALTER TABLE SCORE 
	ADD CONSTRAINT PK_SOCRE_SNO_CNO_PNO PRIMARY KEY(SNO, CNO);
	
-- PK 삭제
-- ALTER TABLE 구문을 이용
-- 제약조건명을 이용한 삭제
ALTER TABLE SCORE 
	DROP CONSTRAINT PK_SOCRE_SNO_CNO_PNO;
...
-- 								   테이블 간의 종속관계를 형성한다. 부모테이블에 존재하는 데이터만 저장가능
...
-- 자식테이블인 EMP_FK1의 DNO으로 사용할 수 없다.
INSERT INTO EMP_FK1
VALUES (3, '장길산', 3);
...
-- FK로 관계가 형성되면 부모 테이블에서 데이터의 수정이나 삭제가 자유롭게 진행되지 않는다.
...
CREATE TABLE SC_STUDENT_FK(
	CNO VARCHAR2(8),
	SNO VARCHAR2(8),
	CONSTRAINT FK_STUDENT_PK_FK_SNO FOREIGN KEY(SNO)
		REFERENCES STUDENT(SNO) ,
	CONSTRAINT FK_STUDENT_PK_FK_CNO FOREIGN KEY(CNO)
		REFERENCES COURSE(CNO) ,
--	CONSTRAINT ST_SCORE_PK_FK PRIMARY KEY(SNO, CNO)
);

ALTER TABLE SC_STUDENT_FK ADD CONSTRAINT ST_SCORE_PK_FK PRIMARY KEY(SNO, CNO);

-- STUDENT 테이블에 SNO로 존재하는 데이터와 COURSE 테이블에 CNO로 존재하는 데이터만 저장할 수 있고
-- 데이터의 묶음이 중복이면 안되고 NULL 값을 허용하지 않는 테이블
-- 같은 학생이 같은 과목을 여러번 수강할 수 없는 테이블이 된다.
INSERT INTO SC_STUDENT_FK
VALUES('913901', '1211');
INSERT INTO SC_STUDENT_FK
VALUES('913901', '1212');
COMMIT;
...
-- N대 N관계(다대다 관계): 두 개의 테이블의 데이터가 여러개 존재하는 n대 n관계가 매핑된 구조
...
-- UK 삭제
-- 제약조건 명으로만 삭제 가능하다.
-- 여러개를 만든 경우 제약조건명마다 삭제해야한다 (ALL삭제 없음, 근데 별로 지울일이 없음)
ALTER TABLE DEPT_UK
...
-- 1-6. DEFAULT : 특정 컬럼의 데이터가 NULL로 오거나 들어오지 않았을 때를 대비해서 기본값을 지정해놓은 제약조건
...
-- LOC(VARCHAR2(10), DEFAULT '서울', NOT NULL) 컬럼을 갖는 FACTORY1을 생성하세요.
...
-- FNO(NUMBER(10), FK(FACTORY1의 FNO), NOT NULL) 컬럼을 갖는 GOODS1을 생성하세요
CREATE TABLE GOODS1(

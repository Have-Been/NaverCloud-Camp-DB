-- PL/SQL (Procedural Langauge extension to SQL) : 오라클에서 제공하는 절차적 언어를 만들기 위한 기능
-- 												   쿼리문에 변수를 선언하고 조건문이나 반복문을 사용할 수 있다
-- PL/SQL에서는 출력하기 위해서 DBMS_OUPUT.PUT_LINE 함수를 이용한다.
...
DECLARE
	-- STUDENT 테이블에 존재하는 칼럼을 모두 가지고 있는 변수 선언
...
-- SELECT 구문을 이용해서 변수값을 할당할 때는 BEGIN문 안에서 할당한다.
DECLARE
	STUDENT_NO STUDENT.SNO%TYPE;
BEGIN
	-- SELECT INTO 구문
...
-- 정교수, 부교수가 아닌 교수는 시간강사입니다를 출력하세요.
...
BEGIN
	...
	ELSE
		DBMS_OUTPUT.PUT_LINE(PROFESSOR_ROW.PNAME || '정교수, 부교수도 아닌 교수는 시간강사입니다.');
	END IF;
END;
...
-- 기본 LOOP문을 이용해서 학생의 학생번호, 학생이름 10개 출력
DECLARE
	STUDENT_ROW STUDENT%ROWTYPE;
	CNT NUMBER := 0;
BEGIN

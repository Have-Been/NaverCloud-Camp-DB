...
-- 과목번호, 과목이름, 담당교수 번호, 담당교수 이름 조회(COURSE, PROFESSOR 테이블 조회)
SELECT CS.CNO
	, CS.CNAME
	, PR.PNO 
	, PR.PNAME 
...
-- 1-1. 등가조인
-- 등가조인은 공통된 칼럼의 데이터가 같은 데이터만을 조회하는 조건
...
-- 학생의 학생번호, 학생이름, 과목번호, 과목이름, 기말고사성적,기말고사 성적의 등급 조회(BETWEEN AND절 사용)
SELECT ST.SNO 
	, ST.SNAME 
	, SC.CNO
	, C.CNAME
...
-- LEET로 기준 테이블을 지정하면 FROM 절에 사용한 테이블이 기준이 되고
...
SELECT ST.SNO
	...
	FROM STUDENT ST
	  ...
	  WHERE ST.SNO  = SC.SNO(+)
	  ORDER BY ST.SNO DESC;
-- LEFT는 WHERE ST.SNO  = SC.SNO(+)
-- RIGHT는 WHERE ST.SNO(+)  = SC.SNO
	 
-- 과목번호, 과목이름, 교수번호, 교수이름을 조회하는 데 담당교수가 배정되지 않은 과목정보도 조회
...
-- 사원의 사원번호, 사원이름, 사수번호, 사수이름 조회하는 데 사수가 배정되지 않은 사원의 정보도 조회
SELECT E.ENO
	, E.ENAME
	, E.MGR
	, EM.ENAME 
...
-- LEFT OUTER JOIN, RIGHT OUTER JOIN, FULL OUTER JOIN에서 OUTER는 생략 가능하다
SELECT E.ENO
	, E.ENAME
	, E.MGR
	, EM.ENAME 
...
-- 과목번호, 과목이름, 교수번호, 교수이름 조회하는 데 아직 담당과목을 배정받지 못한 교수 정보도 조회
SELECT C.CNO 
	, C.CNAME 
	, P.PNO 
	, P.PNAME 
...
-- 사원의 사원번호, 사원이름, 부서번호, 부서이름을 조회하는데 부서를 배정받지 못한 사원의 정보와 사원이 한명도 존재하지 않는 부서의 정보도 함께 조회
SELECT E.ENO 
	, E.ENAME 
	, D.DNO 
	, D.DNAME 
...
-- 4. 다중 테이블 조인
-- 사원의 사원번호, 사원이름, 급여, 급여등급, 부서번호, 부서이름 조회하는 데 부서를 배정받지 않은 사원의 정보도 조회
SELECT E.ENO
	, E.ENAME
	, E.SAL
	, GR.GRADE
...
-- 급여가 3000이상이고 부서번호가 01, 10, 20, 60인 사원의 사원번호, 사원이름, 급여, 급여등급, 부서번호, 
-- 부서이름 조회하는 데 부서를 배정받지 않은 사원의 정보도 조회
-- OUTER JOIN이라서 WHERE 사용 시 조인조건이 달라져 다른 데이터가 나오게 된다.
SELECT E.ENO
	, E.ENAME
	, E.SAL
	, GR.GRADE
